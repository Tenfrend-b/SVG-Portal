...
public class Entrances extends Activity {
    ...
    private void handleEntranceB(Intent intent) {
        ...
        // 从 URI 的 path 部分提取实际路径（MT 管理器直接提供）
        String filePath = fileUri.getPath();
        if (filePath == null || filePath.isEmpty()) {
            Log.e(TAG, "Could not extract path from URI: " + fileUri);
            return;
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) { // Android 11+
            ...
            if (checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                // 提示用户前往 MainActivity 授权
                Toast.makeText(this, "需要存储写入权限，请在设置中授予", Toast.LENGTH_LONG).show();
        ...
        // 调用生成器
        GenPortal.generate(this, filePath);
    }
